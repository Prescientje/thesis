...
\section{Previous Work Related to Hand Hygiene}
...
The authors also introduced the idea of Recurrent Neural Networks (RNNs). These types of networks are better suited for problems which ``involve sequential inputs, such as speech and language.'' They take in each input one at a time in order, going through the algorithm through each input while also accounting for the newer inputs, before outputting a value. These networks have trouble with storing data for a long time, so they are often combined with Long Short-Term Memory Cells (LSTMs), which is a unit that has hidden layers which allow for storing values and remembering them or clearing them based on inputs.
...
For all the tests I ran, I recorded statistics gathered from the confusion matrix. One of the main values one can gather from a confusion matrix is recall, which is $\frac{truepositives}{truepositives + falsenegatives}$. These values are all the averages from the models which took in the data of both the left and right hands as inputs. These values are from the testing portions of 10-fold cross-validated data. As shown in Figure~\ref{recall}, beyond the results of the XYZ model, the Layered model performed the best, maxing out at 71.10\% with a sample length of 100 (= 1 second). The maximum value of the Complex model was 75.07\% at the sample length of 75. 
