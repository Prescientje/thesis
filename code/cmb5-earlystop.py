...
for k in range(1): 
    ...
    while e < epochs:
        ...
        if (e % 20) == 0:
            ...
            print_write("%02d -- %02.10f -- %02.10f -- %02.10f -- %02.10f" % (e, epoch_acc,epoch_acct,epoch_ce,epoch_cet))
        ...
        prev2ce[1] = prev2ce[0]
        prev2ce[0] = epoch_cet
        oldestBoutvalst = olderBoutvalst
        olderBoutvalst = bout_valst
        oldestBoutvals = olderBoutvals
        olderBoutvals = bout_vals
    ...
    final_argt = bout_valst
    final_arg = bout_valst
    if prev2ce[0] > prev2ce[1] and prev2ce[0] > epoch_cet:
        final_argt = olderBoutvalst
        final_arg = olderBoutvals
    elif prev2ce[1] > prev2ce[0] and prev2ce[1] > epoch_cet:
        final_argt = oldestBoutvalst
        final_arg = oldestBoutvals

    bs = accuracy_score(train_bdata_feeder[lout_place].argmax(axis=1),final_arg.argmax(axis=1))
    bst = accuracy_score(test_bdata_feeder[lout_place].argmax(axis=1),final_argt.argmax(axis=1))
    bc = confusion_matrix(train_bdata_feeder[lout_place].argmax(axis=1),final_arg.argmax(axis=1))
    bct = confusion_matrix(test_bdata_feeder[lout_place].argmax(axis=1),final_argt.argmax(axis=1))
    bconfusion_sums += bc
